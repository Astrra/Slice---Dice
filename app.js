...
app.post('/add_record', (req,res) =>{
    ...
    if(!validatePayload(payload, ['name', 'salary', 'currency', 'department', 'sub_department'])){
...
app.get('/summary_stats/department_salary/:department', (req, res) => {
    ...
    const departmentRecords = dataset.filter(record => record.department === requestedDepartment);

    // Extract the salaries from the selected records
    const salaries = departmentRecords.map(record => parseFloat(record.salary));

    if (salaries.length === 0) {
        return res.status(404).json({ error: 'No records found for the specified department' });
    }
...
app.get('/summary_stats/department_sub_department_salary/:department/:sub_department', (req, res) => {
    ...
    const subDepartmentRecords = dataset.filter(record => 
        record.department === requestedDepartment && 
        record['sub_department'] === requestedSubDepartment
    );

    // Extract the salaries from the selected records
    const salaries = subDepartmentRecords.map(record => parseFloat(record.salary));

    if (salaries.length === 0) {
        return res.status(404).json({ error: 'No records found for the specified department and sub-department' });
    }
